...
folder are mapped to the application's root namespace, 'SppAppClassic'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "SppAppClassic"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'SppAppClassic' in the same way
as `"SppAppClassic/sass/src"`.
